...
import sys
from pretrain.datamodule import collate_fn
...
class GlobalForecastDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        train_files = [os.path.join(root_dir, "train", f) for f in os.listdir(os.path.join(root_dir, "train")) if
                       f.endswith(".npz")]
        val_files = [os.path.join(root_dir, "val", f) for f in os.listdir(os.path.join(root_dir, "val")) if
                     f.endswith(".npz")]
        test_files = [os.path.join(root_dir, "test", f) for f in os.listdir(os.path.join(root_dir, "test")) if
                      f.endswith(".npz")]
        self.lister_train = list(IterableWrapper(train_files))
        self.lister_val = list(IterableWrapper(val_files))
        self.lister_test = list(IterableWrapper(test_files))
    ...
    def get_iterations_per_epoch(self):
        if self.data_train is None:
            raise ValueError("DataModule is not set up yet. Call `setup()` before calling `get_iterations_per_epoch()`.")

        dataset_size = 0
        for file_path in self.lister_train:
            data = np.load(file_path)
            dataset_size += len(data)

        iterations_per_epoch = (dataset_size + self.hparams.batch_size - 1) // self.hparams.batch_size
