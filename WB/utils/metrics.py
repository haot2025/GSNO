...
import models.quadrature as harmonics


def l2loss_sphere(prd, tar, vars, relative=True, squared=False, reduction="sum"):
    B, C, H, W = prd.shape
    w_quad = torch.tensor(harmonics.legendre_gauss_weights(H, -1, 1)[1], device=prd.device, dtype=prd.dtype)
...
def l2loss_sphere_no_sine(prd, tar, relative=True, squared=False, reduction="sum"):
    B, C, H, W = prd.shape
    w_quad = torch.tensor(harmonics.legendre_gauss_weights(H, -1, 1)[1], device=prd.device, dtype=prd.dtype)
    ...
    if relative:
        loss = loss / (sphere_weights * tar ** 2).sum(dim=(-1, -2), keepdim=True)
